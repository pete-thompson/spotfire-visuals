...
//    firstTimeSetup: a function that will be called the first time a viz is to be renderd, same parameters as render
...
//                      { selector: jQuery selector for objects of interest, type: the algorithm }
//                algorithms (which all rely on attching the Spotfire row hint to the element's "data-id" attribute) are:
...
exports.SetupViz = function (info) {
  var defaults = { defaultConfig: {}, mark: exports.markType.none, configOptions: [], configuratorTitle: '', configuratorInstructions: '', renderOnResize: false }
  vizInfo = {}
...
window.renderCore = function (data) {
  ...
  try {
    ...
    if (firstTime) {
      ...
      JSVizConfigurator.SetupConfigurator(vizInfo.configuratorTitle, vizInfo.configuratorInstructions, vizInfo.configOptions, config, vizInfo.defaultConfig, data.static)

      // Placeholder for any messaage
      messageDiv = $('<div>')
        .appendTo('body')
        .css('width', '100%')
        .css('height', '100%')
    ...
    JSVizConfigurator.SetColumns(vizInfo.configOptions, data.columns)
    ...
    exports.showMessageInstead('An error occured: ' + e)
  } finally {
...
function setupMarking () {
  if (!markingSetup) {
    ...
    $('body').on('mousedown', function (mouseDownEvent) {
      if (!JSVizConfigurator.ConfiguratorVisible() && vizInfo.mark !== 'none') {
